...
st.set_page_config(page_title="Omni-Architect v28.0", layout="wide", page_icon="🏗️")
    ...
    for p in parts:
        try:
            ...
            with st.container(border=True):
                ...
                st.code(content, language="yaml" if any(x in fname for x in [".yaml", ".yml", ".tf"]) else "dockerfile")
        except: continue
...
# --- 4. MAIN DASHBOARD ---
st.title("🛡️ DevSecOps Omni-Architect v28.0")

if not active_files:
    st.info("👈 Select files in the sidebar to begin.")
else:
    tabs = st.tabs(["🏗️ Infra", "🔭 Observability", "🛡️ Security", "⛓️ Pipelines", "🚀 Execution"])
    ...
    with tabs[2]: # SECURITY/FINOPS
        s1, s2 = st.columns(2)
        if s1.button("Harden (Security)", use_container_width=True):
            st.session_state.state['infra_out'] = ask_ai(f"Secure this: {st.session_state.state['infra_out']}")
            st.rerun()
        if s2.button("FinOps (Cost)", use_container_width=True):
            st.session_state.state['infra_out'] = ask_ai(f"Optimize cost for: {st.session_state.state['infra_out']}")
            st.rerun()

    with tabs[3]: # PIPELINES
        st.subheader("CI/CD Pipeline Generator")
        p_type = st.selectbox("Pipeline Type:", ["GitHub Actions", "GitLab CI", "Jenkins Pipeline", "Tekton"])
        if st.button(f"Generate {p_type} Workflow", type="primary", use_container_width=True):
            prompt = f"Create a {p_type} pipeline for {active_files} that includes Docker build, Trivy security scan, and K8s deploy. Use ---FILE: pipeline.yaml---"
            st.session_state.state['pipe_out'] = ask_ai(prompt)
